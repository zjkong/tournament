...
def playerStandings():
    ...
    conn = connect()
    cursor = conn.cursor()
    cursor.execute("SELECT id, player_names, wins, matches from players natural join records order by wins desc")
    results = cursor.fetchall()
    return results
...
def reportMatch(winner, loser):
    ...
    conn = connect()
    cursor = conn.cursor()
    cursor.execute("UPDATE records SET wins = wins + 1, matches = matches + 1 where id = %s",(winner,));
    cursor.execute("UPDATE records SET matches = matches + 1 where id = %s", (loser,));
    conn.commit()
